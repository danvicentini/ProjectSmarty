...
class laboratorio extends controller
{
    ...
    { 
        $dataLaboratorio = new laboratorioModel();
        ...
        $this->template->fetchJS('/files/js/laboratorio/listar.js');
    ...
    public function cadastro()
    {
        ...
        if(!$vis)
        {
            $this->template->fetchJS('/files/js/laboratorio/cadastro.js');
        }
       
        //editando consumidor
        $lab_id = $this->getParam("lab_id");
        //Pegando clientes do banco
				
        $dataLaboratorio = new laboratorioModel();
				if($lab_id>0){
        	  
            $arrLaboratorio = array();
            $arrLaboratorio = $dataLaboratorio->getLaboratorios($lab_id,null);
						//echo " OI2 ";        
						$this->smarty->assign('lab_id', $lab_id);
						$this->smarty->assign('arrLaboratorio', $arrLaboratorio[0]);
        ...
        $this->template->setTitle('Qualibrás - Cadastro de Servi&ccedil;o');
        $this->template->run();

        if($vis)
        {
            $this->smarty->display('laboratorio/visualizar.html');
        }
        else
        {
            $this->smarty->display('laboratorio/cadastro.html');
        }
    }

    public function gravar_dados()
    {
				$dadosLaboratorio['nom_laboratorio'] = addslashes(@$_POST['nom_laboratorio']);
        $dadosLaboratorio['descricao'] = addslashes(@$_POST['descricao']);
        
        $laboratorioModel = new laboratorioModel();

        //UPDATE
        if(isset($_POST['lab_id']) && !empty($_POST['lab_id'])){
        	$laboratorioModel->upd($dadosLaboratorio, $_POST['lab_id']);
					$arrReturn['msg'] = 'Dados atualizados com sucesso!';
        }
        //INSERT
        else{
					$_POST['lab_id'] = $laboratorioModel->create($dadosLaboratorio);
        ...
        $arrReturn['redirect'] = '/laboratorio/listar';

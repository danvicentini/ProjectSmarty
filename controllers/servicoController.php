...
class servico extends controller
{
    ...
    public function listar()
    {
        ...
        $this->template->fetchJS('/files/js/servico/listar.js');
    ...
    public function cadastro()
    {
        ...
        if(!$vis)
        {
            $this->template->fetchJS('/files/js/servico/cadastro.js');
        }
       
        //editando consumidor
        $srv_id = $this->getParam("srv_id");
        ...
        $dataServico = new servicoModel();
            ...
            $arrServico = array();
            $arrServico = $dataServico->getServico($srv_id,null);
						//echo " OI2 ";        
						$this->smarty->assign('srv_id', $srv_id);
						$this->smarty->assign('arrServico', $arrServico[0]);
        ...
        if($vis)
        {
            $this->smarty->display('servico/visualizar.html');
        }
        else
        {
            $this->smarty->display('servico/cadastro.html');
        }
    }

    public function gravar_dados()
    {
				$dadosServico['nom_servico'] = @$_POST['nom_servico'];
        $dadosServico['descricao'] = @$_POST['descricao'];
        
        $servicoModel = new servicoModel();

        //UPDATE
        if(isset($_POST['srv_id']) && !empty($_POST['srv_id'])){
        	$servicoModel->upd($dadosServico, $_POST['srv_id']);
					$arrReturn['msg'] = 'Dados atualizados com sucesso!';
        }
        //INSERT
        else{
					$_POST['srv_id'] = $servicoModel->create($dadosServico);
        ...
        $arrReturn['redirect'] = '/servico/listar';
